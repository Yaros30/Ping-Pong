            ...
            while True:
                ...
                with self.lock:
                    if data == "UP":
                        ...
                        self.paddles[pid] = min(HEIGHT - 200, self.paddles[pid] + PADDLE_SPEED)
                    elif data == "RESTART":
                        if self.game_over:
                            self.reset_game_state()
    ...
    def ball_logic(self):
        ...
        while not self.game_over:
            with self.lock:
                ...
                if (self.ball['x'] <= 40 and self.paddles[0] <= self.ball['y'] <= self.paddles[0] + 200) or \
                   (self.ball['x'] >= WIDTH - 40 and self.paddles[1] <= self.ball['y'] <= self.paddles[1] + 200):
                    self.ball['vx'] *= -1
